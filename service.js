...
module.exports = function(env) {
	...
	db.init(env, function(err) {
		...
		db.get(env.ldpBase, 'application/ld+json', function(err, document) {
	...
	function readShapes(json, callback){
		...
		for(var i = 0; i < graph.length; i++){
			...
			if(graph[i]["creation"]){
				if(graph[i]["resourceShape"]){
					if(graph[i]["resourceType"]){

						// var write = fs.createWriteStream("../../shape_files/"+file_name+".json");

						if(graph[i]["resourceShape"].includes("http:")){

							try{
								http.get(graph[i]["resourceShape"], function(response){
									console.log("RESPONSE: " + response);
									fs.writeFile("../../shape_files/"+graph[i]["creation"]+i+".json", response);
								});
							}catch(err){
							...
							try{
								https.get(graph[i]["resourceShape"], function(response){
									console.log("RESPONSE: " + response);
									fs.writeFile("../../shape_files/"+graph[i]["creation"]+i+".json", response);
								});
							}catch(err){
	...
	function findBlankNodes(blank_subject, main_uri, triples, serialize, first_time, callback){
	    var new_triples = [];

	    return assignURI(main_uri, "/", first_time, function(err, uri, first_time){
	    	...
	    	if(err){
			          ...
			          if(triples[i].object.includes("_:b")){
			               ...
			               obj = findBlankNodes(triples[i].object, uri, triples, serialize, first_time, callback);
							...
							jsonld.parse(file, function(err, ex_triples){
								...
								findBlankNodes(obj, uri, ex_triples, serialize, true, callback);
							});
		     ...
		     serialize(new_triples, function(err, content_type, result){
		        ...
		        db.put(uri, result, 'application/ld+json', function(err){
		      		if(err){
		          		console.log(err.stack);
		              	callback(err);
		           	}
		...
		// generates and reserves a unique URI with base URI 'container'
		function uniqueURI(container, first_time, callback) {

			if(!first_time){
				container = addPath(container, 'res' + Date.now());
			}
			first_time = false;
		...
		// reserves a unique URI for a new subApp. will use slug if available,
		// but falls back to the usual naming scheme if slug is already used
		function assignURI(container, slug, first_time, callback) {
/*
			if (slug) {
				var candidate = addPath(container, slug);
...
*/	
				//console.log("CANDIDATE: " + container);
				return uniqueURI(container, first_time, callback);
			//}
		}
